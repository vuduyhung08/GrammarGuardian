            ...
            <div class="container all-content" id="hanging-icons">
                <div class="row">
                    <div class="header">
                        ...
                        <p>Grammar Guardian’s free essay-checking tool will help you review your papers for grammatical
                            mistakes, unclear
                        ...
                        <div class="col-md-12 d-flex">
                            <div class="text-origin">
                                <div style="width: 100%; height: 100%; margin-right: 10px">
                                    <textarea required id="text" name="text" class="check-essay" oninput="countWords()"
                                        placeholder="Start writing here.">${text != null ? text : ''}</textarea>
                                </div>
                                <div>
                                    <div>
                                        <c:choose>
                                    ...
                                    <div>
                                        ...
                                        <select id="check-type" name="check-type" class="styled-select">
                                            <option value="all" ${checkType=='all' ? 'selected' : '' }>Check All
                                            </option>
                                            <option value="spell" ${checkType=='spell' ? 'selected' : '' }>Check
                                                Spelling</option>
                                            <option value="grammar" ${checkType=='grammar' ? 'selected' : '' }>Check
                                                Grammar</option>
                                        </select>
                                    </div>
                                    <div>
                                        ...
                                        <c:choose>
                                            <c:when test="${USER != null}">
                                                <p class="login">
                                                    Note: This feature requires registration to get more time to check
                                                    your essay. (<b>${USER.checkFreeTime} times</b> remaining)
                                                </p>
                                            </c:when>
                                            <c:otherwise>
                                                <p class="login">Already have an account?
                                                    <a href="${pageContext.request.contextPath}/LoginController">Log
                                                        in</a>
                                                </p>
                                            </c:otherwise>
                                        </c:choose>
                            ...
                            <div class="main-content" style=" margin: 0;
                             ...
                             padding: 0;">
                                <c:if test="${segments != null}">
                                        ...
                                        <table class="styled-table">
                                            ...
                                            <tbody class="over-content">
                                                <c:forEach var="match" items="${matches}">
                                                    <tr>
                                                        <td>${match.message}</td>
                                                        <td style="color: red">${text.substring(match.fromPos,
                                                            match.toPos)}</td>
                                                        <td>${match.fromPos}-${match.toPos}</td>
                                                        <td style="color: green">
                                                            <c:forEach var="suggestion"
                                                                items="${match.suggestedReplacements}">
                                    ...
                                    <a class="save-button" data-bs-toggle="modal" data-bs-target="#save-post">Save your
                                        essay</a>
                                    <a class="edit-button"
                                        href="${pageContext.request.contextPath}/HomeController">Continue to check</a>
                                </c:if>
                            </div>
                        </div>
                    </form>
                    <!-- Modal -->
                    <div class="modal fade" id="save-post" tabindex="-1" aria-labelledby="exampleModalLabel"
                        aria-hidden="true">
                        <div class="modal-dialog">
                            <form class="modal-content" action="${pageContext.request.contextPath}/grammar-checker"
                                method="POST">
                                ...
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <li class="page-item"><a class="page-link"
                                                href="HomeSearchController?search=${search}&index=${selectedPage-1}">«</a>
                                        </li>
                                    </c:otherwise>
                                </c:choose>
                                <c:forEach var="i" begin="1" end="${endP}">
                                    <li class="page-item ${i == selectedPage ? " active" : "" }"> <a class="page-link"
                                            href="HomeSearchController?search=${search}&index=${i}">${i}</a>
                                    <li>
                                </c:forEach>
                                <c:choose>
